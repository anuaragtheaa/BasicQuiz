function onpageload(){
    ...
    var ans = JSON.parse(window.localStorage.getItem('ans'));
    for(var i=0;i<10;i++){
        if(ans[i]==alp[uans[i]]){
            cr++;
        }
    ...
    document.getElementById('attempted').innerHTML=at;
    document.getElementById('correct').innerHTML=cr;
    document.getElementById('total').innerHTML=cr*10;
    document.getElementById('percentage').innerHTML=cr*10+'%';
    if(cr>4){
        document.getElementById('resul').innerHTML='Pass';
    }    
    else{
        document.getElementById('resul').innerHTML='Fail';
    }
}
function signup(){
